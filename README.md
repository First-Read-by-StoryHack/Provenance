...
## How this file gets published, and who owns it

**Owner: LT.** Before 2026-09-17 this step had no owner and no command, which is why the site and
this mirror drifted twice. It is two commands now.

```
npm run mirror:build      # regenerate manifest.json from src/lib/provenance/how-to-verify.ts
npm run mirror:publish    # clone this repo, copy mirror/, commit, push to main
```

`mirror:publish` **refuses to run** if `mirror/manifest.json` is not what the generator would
produce, because publishing a stale file confidently is the failure it exists to prevent. Add
`--dry-run` to see what would change without touching anything. It uses the git credentials you
already push with and never reads or stores a token.

**Then confirm, because a successful push is not the same as a served file:**

```
npx tsx scripts/check-published-mirror-agrees.ts
```

**This runs on its own too.** It is a step in `.github/workflows/live-copy.yml`, after every deploy
and every 30 minutes, retried three times so a network blip is not read as a divergence. The
schedule matters more than the push trigger here: this divergence is created by a step somebody
forgets, not by a commit.

**Why a push can say "everything up to date" while this file is stale.** The app and this mirror
are different repositories. `git push` in the product repo is telling the truth about the only repo
it knows about. That sentence has now cost two incidents.

**Settled, and not to be raised again:** the lowercase `provenance` path in the `mirrors` array is
correct. Both casings resolve; it has been checked three times.
